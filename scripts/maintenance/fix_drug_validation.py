#!/usr/bin/env python3
"""
Script to fix drug validation to be more inclusive of valid drug names.
"""
...
from src.models.entities import Drug, Company, ClinicalTrial

def fix_drug_validation():
    """Fix drug validation to be more inclusive."""
    db = get_db()
    
    try:
        print("🔧 Fixing drug validation to be more inclusive...")
        ...
        for company in companies:
            ...
            # Filter drugs using more inclusive validation
            valid_drugs = []
            for drug in drugs:
                if _is_valid_drug_name_improved(drug.generic_name):
                    valid_drugs.append(drug.generic_name)
            
            # Remove duplicates within same company and sort
        ...
        for i, drug in enumerate(sorted(all_drugs)[:15], 1):
            print(f"  {i:2d}. {drug}")
        
        if len(all_drugs) > 15:
            print(f"  ... and {len(all_drugs) - 15} more")
            
    except Exception as e:
        print(f"❌ Error fixing drug validation: {e}")
        raise
    finally:
        db.close()

def _is_valid_drug_name_improved(name: str) -> bool:
    """Improved drug name validation that's more inclusive."""
    import re
