#!/usr/bin/env python3
"""
Script to fix drug name capitalization without removing duplicates.
"""
...
def fix_drug_capitalization():
    """Fix drug name capitalization without removing duplicates."""
    db = get_db()
    
    try:
        print("🔧 Fixing drug name capitalization...")
        ...
        for drug in drugs:
            if drug.generic_name:
                ...
                if original_name != drug.generic_name:
                    updated_count += 1
                    print(f"  Updated: '{original_name}' → '{drug.generic_name}'")
        
        # Commit changes
        db.commit()
        
        print(f"\n✅ Updated capitalization for {updated_count} drugs")
        ...
        print(f"❌ Error fixing drug capitalization: {e}")
        db.rollback()
        raise
    finally:
        db.close()
